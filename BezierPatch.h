    ...
    void CreateGeometry(const std::vector<glm::vec3>& controlPoints, int nSubDivisions)
    {
        ...
        for (int i = 0; i < nSubDivisions; i++)
        {
            for (int j = 0; j < nSubDivisions; j++)
            {
                ...
                EvaluateBezierPoint(u1, v0, controlPoints, pos[3], norm[2], tang[3]);
                ...
                {
                    int ivert = indices[k];
                    positions.push_back(pos[ivert]);
                    normals.push_back(norm[ivert]);
                    uvs.push_back(uv[ivert]);
                    tangents.push_back(tang[ivert]);
